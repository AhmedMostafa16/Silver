...
impl Default for TransportConfig {
    fn default() -> Self {
        TransportConfig::Tcp { addr: ([127, 0, 0, 1], 8080).into() }
    }
}
...
impl Builder {
    ...
    pub fn finish(&mut self) -> Result<Incoming, Error> {
        ...
        let kind = match builder.config {
            TransportConfig::Tcp { addr } => IncomingKind::Tcp(
                TcpListener::bind(&addr)?.incoming(),
            ),
            #[cfg(unix)]
            TransportConfig::Uds { path } => IncomingKind::Uds(
...
mod tls {
    ...
    fn load_certs(path: &PathBuf) -> Result<Vec<Certificate>, Error> {
        ...
        let certs = pemfile::certs(&mut reader).map_err(|_| {
            format_err!("failed to read certificates")
        })?;
        Ok(certs)
    }
    ...
    fn load_key(path: &PathBuf) -> Result<PrivateKey, Error> {
        ...
        let keys = pemfile::pkcs8_private_keys(&mut reader).map_err(|_| {
            format_err!("failed to read private key")
        })?;
        if keys.is_empty() {
